...
real*8 ::  t_try
 ...
 do in_N=1,25
	Itable_n_N(in_N) = &
		 (-4.0d0+dble(in_N-1)/dble(25-1)*(4.0d0))
 enddo
...
S=0.0d0
indx=0
!~ temp_array =temp_array/100.0d0

!~ t_try = matter_temperature/100.0d0

!~ write(*,*) t_try,temp_array
if ( matter_temperature .GT. maxval(temp_array) .or. matter_temperature .LT. minval(temp_array) &
	...
	.or.  n_N_fm .GT.maxval(Itable_n_N) .or. n_N_fm .LT. minval(Itable_n_N)  & 
...
!~ 						,25, 25,om_array, Ye_array,Itable_n_N,temp_array)
	call intp3d_many (  omega, Ye,n_N_fm, eas, 1, gang_table,40,26,25,25&
	...
	dx = 0.5d0
!~ 	dx = 2.0d0
	indx= 1+ NINT((matter_temperature-temp_array(1))*dx)
!~ 	write(*,*) matter_temperature, n_N_fm, Ye,omega
!~ 	write(*,*) minval(temp_array),minval(Itable_n_N),minval(Ye_array)
!~ 	write(*,*) temp_array(1),Itable_n_N(1),Ye_array(1),om_array(indx)
!~ 	write(*,*) "plop",gang_table(6,16,16,indx)
!~ 	write(*,*) 
!~ 	write(*,*) eas
!~ 	write(*,*) eas(indx), omega
!~ 	write(*,*)
!~ 	write(*,*) "plop", gang_table(:,2,2,2)
	S = eas(indx)
!~ 	write(*,*) matter_temperature, indx,temp_array(indx)
!~ 	write(*,*) 
!~ 	write(*,*) matter_temperature,n_N_fm,Ye,10**omega
!~ 	write(*,*) temp_array(6),Itable_n_N(21),Ye_array(16)
endif
...
if (Phi0_nn .LT. 0.0d0) then 
	...
	write(*,*) eas
	write(*,*) "arg", shape(eas), shape(gang_table)
	write(*,*)
	write(*,*) om_array
	write(*,*) temp_array
	write(*,*) Ye_array
