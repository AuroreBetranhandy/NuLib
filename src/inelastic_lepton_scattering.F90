!-*-f90-*-
function NES_Phi0_ThompsonBruenn(nu_energy_in,nu_energy_out,matter_eta,matter_temperature,neutrino_species,lepton) result(Phi0)
  ...
  real*8, intent(in) :: matter_eta !lepton chemical potential / Temp, dimensionless
  ...
  integer, intent(in) :: lepton !lepton species, 1=electron, 2=muon
  ...
  integer :: i
  real*8 :: lepton_x,fem,feother !the underscore x denotes /Temp
  ...
  if (lepton .gt. 2) then 
	stop "inelastic kernel: non existing lepton"
  endif

  do i=1,npoints
     !must take interval change into account
     lepton_x = width * (float(i)-0.5d0) !MeV/Temp
     fem = fermidirac_dimensionless(lepton_x,matter_eta)
     feother = 1.0d0-fermidirac_dimensionless(lepton_x+nu_energy_in_x - &
          nu_energy_out_x,matter_eta)
     Phi0 = Phi0 + &
          fem*feother*NES_H0_ThompsonBruenn(nu_energy_in_x,nu_energy_out_x, &
          lepton_x,neutrino_species,lepton)
  enddo
  Phi0 = Phi0*width

  !constants that do not have to do with the integral over lepton energy
...
function NES_Phi1_ThompsonBruenn(nu_energy_in,nu_energy_out,matter_eta, &
     matter_temperature,neutrino_species,lepton) result(Phi1)
  ...
  real*8, intent(in) :: nu_energy_in !incoming neutrino energy, MeV
  real*8, intent(in) :: nu_energy_out !outgoing neutirno energy, MeV
  real*8, intent(in) :: matter_eta !lepton chemical potential / T, dimensionless
  real*8, intent(in) :: matter_temperature !temperature, MeV
  ...
  do i=1,npoints
     !must take interval change into account
     lepton_x = bound/float(npoints) * (float(i)-0.5) !MeV/Temp
     fem = fermidirac_dimensionless(lepton_x,matter_eta)
     feother = 1.0d0-fermidirac_dimensionless(lepton_x+nu_energy_in_x-nu_energy_out_x,matter_eta)
     Phi1 = Phi1 + &
          fem*feother*NES_H1_ThompsonBruenn(nu_energy_in_x,nu_energy_out_x,lepton_x,neutrino_species,lepton)
  enddo
  Phi1 = Phi1*width
...
function NES_H0_ThompsonBruenn(nu_energy_in,nu_energy_out,lepton_energy,neutrino_species,lepton) result(H0)
  ...
  real*8, intent(in) :: lepton_energy !lepton energy, MeV, or reduced energy
  integer, intent(in) :: lepton !lepton specie: 1=electron, 2=muon
  ...
  H0 = H0_constants(1,neutrino_species,lepton)*H0I + H0_constants(2,neutrino_species,lepton)*H0II !same coefficients as e-e+ annhilation
contains
...
function NES_H1_ThompsonBruenn(nu_energy_in,nu_energy_out,lepton_energy,neutrino_species,lepton) result(H1)
  ...
  H1 = H0_constants(1,neutrino_species,lepton)*H1I + H0_constants(2,neutrino_species,lepton)*H1II !same coefficients as e-e+ annhilation
