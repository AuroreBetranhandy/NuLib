...
!#################################################################
    subroutine initialize_nulib(neutrino_scheme_in,number_species_in,number_groups_in)
      ...
      if (neutrino_scheme.eq.1) then
         ...
         if ((add_numu_emission_bremsstrahlung.neqv.add_anumu_emission_bremsstrahlung).or. &
              (add_numu_emission_bremsstrahlung.neqv.add_nutau_emission_bremsstrahlung).or. &
              (add_numu_emission_bremsstrahlung.neqv.add_anutau_emission_bremsstrahlung)) then
         ...
         if ((add_numu_emission_bremsstrahlung.neqv.add_nutau_emission_bremsstrahlung).or. &
              (add_anumu_emission_bremsstrahlung.neqv.add_anutau_emission_bremsstrahlung)) then
...
!~ 		
    ...
    !|phi^{prod/annihl}_{0/1} from Hannestad & Raffelt 1998
    subroutine single_bremsstrahlung_kernel_point_return_all_Hannestad(iin,n_N,temperature, &
      ...
      if (neutrino_scheme.eq.1) then
         ...
         stop "single_bremsstrahlung_kernel_return_all:incorrect neutrino scheme"
      endif

      if (size(Phi0s,1).ne.number_local_species) then
         stop "single_bremsstrahlung_kernel_return_all:provided array has wrong number of species "
      endif
      if (size(Phi0s,2).ne.number_groups) then
         stop "single_bremsstrahlung_kernel_return_all:provided array has wrong number of groups"
      endif
      if (size(Phi0s,3).ne.2) then
         stop "single_bremsstrahlung_kernel_return_all:provided array has wrong number of kernels"
      endif
    ...
    end subroutine single_bremsstrahlung_kernel_point_return_all_Hannestad

    !calcualates the expansion of the bremsstrahlung kernal, These are
    !|phi^{prod/annihl}_{0/1} from Kuroda et al. 2016 modified via s_kl from Raffelt & Seckel 1995
    subroutine single_bremsstrahlung_kernel_point_return_all_Kuroda(iin,n_N,temperature, &
         Phi0s,neutrino_local_scheme)
